...
public class UpdateOrgQProfilesToPointToBuiltInProfiles extends DataChange {
  ...
  protected void execute(Context context) throws SQLException {
    ...
    Table<String, String, String> builtInRulesProfiles = retrieveBuiltInRulesProfiles(context);
    ...
    massUpdate.select("select oqp.uuid, rp.language, rp.name " +
      ...
      "   and rp.is_built_in = ? " +
      "   and rp.user_updated_at = ?")
      .setString(1, defaultOrganizationUuid)
      .setBoolean(2, false)
      .setLong(3, null);

    massUpdate.update("update orq_qprofiles " +
      "set rules_profile_uuid = ? " +
      "where uuid=?").execute((row, update) -> {

        String orgQProfileUuid = row.getString(1);
        String language = row.getString(2);
        String name = row.getString(3);
        if (!builtInRulesProfiles.contains(language, name)) {
          return false;
        }

        update.setString(1, builtInRulesProfiles.get(language, name));
  ...
  private static Table<String, String, String> retrieveBuiltInRulesProfiles(Context context) throws SQLException {
    // Name, Language, RulesProfileUuid
