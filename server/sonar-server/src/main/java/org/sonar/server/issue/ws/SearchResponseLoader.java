...
public class SearchResponseLoader {
  ...
  private List<IssueDto> loadIssues(SearchResponseData preloadedResponseData, Collector collector, DbSession dbSession) {
    ...
    Set<String> issueKeysToLoad = copyOf(difference(collector.getIssueKeys(), preloadedIssueKeys));
    if (issueKeysToLoad.isEmpty()) {
      return preloadedIssues;
    }

    List<IssueDto> loadedIssues = dbClient.issueDao().selectByOrderedKeys(dbSession, issueKeysToLoad);
  ...
  public static class Collector {
    ...
    public Collector(EnumSet<SearchAdditionalField> fields, Set<String> issueKeys) {
      this.fields = fields;
