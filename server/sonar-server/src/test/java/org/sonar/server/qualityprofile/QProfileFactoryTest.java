...
public class QProfileFactoryTest {
  ...
  public void checkAndCreateCustom() {
    ...
    assertThat(db.getDbClient().qualityProfileDao().selectAll(dbSession, organization)).extracting(QProfileDto::getKee).containsExactly(profile.getKee());
  }
  ...
  public void delete_builtin_profile() {
    ...
    verifyCallActiveRuleIndexerDelete(profile.getKee());

    // remove only from org_qprofiles
    assertThat(db.getDbClient().qualityProfileDao().selectAll(dbSession, org)).isEmpty();
  ...
  private void verifyCallActiveRuleIndexerDelete(String... profileUuids) {
    Class<Set<String>> setClass = (Class<Set<String>>) (Class) Set.class;
    ArgumentCaptor<Set<String>> setCaptor = ArgumentCaptor.forClass(setClass);
    verify(activeRuleIndexer).deleteByProfileKeys(setCaptor.capture());
    assertThat(setCaptor.getValue()).containsExactlyInAnyOrder(profileUuids);
  }
